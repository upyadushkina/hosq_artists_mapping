...
# --- Добавляем кастомный JavaScript для прозрачности других узлов ---
...
document.addEventListener("DOMContentLoaded", function() {{
    var network = window.networks[Object.keys(window.networks)[0]];
    network.on("click", function (params) {{
        ...
        network.body.data.nodes.update(allNodes.map(node => {{
            return {{ id: node.id, opacity: connectedNodes.includes(node.id) || node.id === selectedNode ? 1 : 0.1 }};
        }}));

        network.body.data.edges.update(allEdges.map(edge => {{
            var isConnected = connectedNodes.includes(edge.from) || connectedNodes.includes(edge.to);
            return {{
                id: edge.id,
                color: {{
                    color: isConnected ? '{EDGE_HIGHLIGHT_COLOR}' : '{EDGE_COLOR}',
                    opacity: isConnected ? {EDGE_HIGHLIGHT_OPACITY} : 0.1
                }}
            }};
        }}));
    }});
